...
bonds_simu_real(:,1) =  - betta.*( funeval(parexp_planner1,fspace,[gam1 0])  ...
    - gam1*funeval(parexp1,fspace,[gam1 0]));%
...
%                 funeval(parexp_planner1,fspace,  [phi1_simuNext_s1_real(:,i)   zeta_simu_real(:,i+1)  ] ) - ...
%                 phi1_simuNext_s1_real(:,i).* funeval(parexp1,fspace,   [phi1_simuNext_s1_real(:,i)   zeta_simu_real(:,i+1) ] ) + ...
%                 funeval(parlambda1,fspace,   [phi1_simuNext_s1_real(:,i)   zeta_simu_real(:,i+1) ] )./betta.*...
%                 (nu./funeval(para1,fspace,   [phi1_simuNext_s1_real(:,i)   zeta_simu_real(:,i+1) ] )).*...
%                 (funeval(parexp1,fspace,   [phi1_simuNext_s1_real(:,i)   zeta_simu_real(:,i+1) ] ) - ...
%                 (phi1_simuNext_s1_real(:,i).^((1-sig)./sig))./(1-sig) + ...
%                 alpha.*(funeval(para1,fspace,   [phi1_simuNext_s1_real(:,i)   zeta_simu_real(:,i+1) ] ).^epsil))) + ...
%                 prob1_s2_simu_real(:,i).*(...
%                 (zeta_simu_real(:,i+1)./phi1_simuNext_s2_real(:,i))./betta  + ...
%                 (s(1) -s(2) +funeval(parexp_planner1,fspace,  [phi1_simuNext_s2_real(:,i)   zeta_simu_real(:,i+1)  ] )) - ...
%                 phi1_simuNext_s2_real(:,i).* funeval(parexp1,fspace,  [phi1_simuNext_s2_real(:,i)   zeta_simu_real(:,i+1)  ] ) + ...
%                 funeval(parlambda1,fspace,  [phi1_simuNext_s2_real(:,i)   zeta_simu_real(:,i+1)  ] )./betta.*...
%                 ((-nu2.*(funeval(para1,fspace,  [phi1_simuNext_s2_real(:,i)   zeta_simu_real(:,i+1)  ] ).^(nu2-1)))./...
%                 (   1 - funeval(para1,fspace,  [phi1_simuNext_s2_real(:,i)   zeta_simu_real(:,i+1)  ] ).^nu2)).*...
%                 (funeval(parexp1,fspace,  [phi1_simuNext_s2_real(:,i)   zeta_simu_real(:,i+1)  ] ) - ...
%                 (phi1_simuNext_s2_real(:,i).^((1-sig)./sig))./(1-sig) + ...
%                 alpha.*(funeval(para1,fspace,  [phi1_simuNext_s2_real(:,i)   zeta_simu_real(:,i+1)  ] ).^epsil))) );
        end;
    end  % clear sssval;

    c_simu_real = c1_simu_real;% (( phi1_simu_real(:,1:end-1)).^(1/sig)); % ((omega1 + phi1)./eta).^sig; %
    ...
    prob_s1_simu = a1_simu.^nu;%1 - exp(-rho.*a1_simu);
